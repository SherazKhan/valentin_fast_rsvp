...
%[key time code]  = collect_response(cfg,delay)
...
while (sum(resp) == 0 || isempty(code)) && t <= delay
    ...
    if ~isempty(exp_code)
        switch exp_code
            ...
            case cfg.escape, sca;break;
        end
    end

    if strcmp(cfg.port,'keyboard')              % if keyboard input
...
if cfg.until_release
    ...
    while sum(resp_release) ~= 0
        if strcmpi(cfg.port,'keyboard')              % if keyboard input
